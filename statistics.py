...
def population_statistics(feature_desc, data, trt, trg, threshold, is_abv, stats_funcs):
    """
       ...
       :return:
       """
    values = {x for x in data[trt] if (is_abv and x > threshold) or (not is_abv and x <= threshold)}
    print(feature_desc)
    print_details(filter_by_feature(data, trt, values)[0], [trg], stats_funcs)
